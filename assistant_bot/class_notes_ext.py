...
class Notes(UserDict):
    ...
    def search(self, search_str, category):
        ...
        for key, record in self.data.items():
            if category == 1:
                ...
                if any(search_str in str(tag) for tag in record.self.tags):
                    results[key] = record
        return results
    ...
    def sort(self, category):
        if category == '1': #Datetime
            ...
            results = sorted(notes.data.items(), key=lambda item: [tag.value for tag in item[1].self.tags])
        return results
...
class Notes_Storage:
    ...
    def change_note(self, index):
        ...
        if self.notes.data[index]:
            ...
            note = record.note
            self.tags = record.self.tags
            while True:
                category = input('Press "1" to edit notes, "2" - to edit self.Tags#: ')
            ...
            if category == '1':
                ...
                self.tags = input(f'Current #self.Tags({record.self.tags}): ')
                self.tags = [Tag(tag) for tag in self.tags.split(' ')]
            record = Note_Record(index, note, date, self.tags)
            self.notes.add_record(record)
    ...
    def sort_notes(self, *args):
        while True:
            category = input('Press "1" to sort by date, "2" - to sort by index, "3" - to sort by #self.Tags: ')
    ...
    def search_notes(self):
        while True:
            category = input('Press "1" to search in notes, "2" - in #self.Tags: ')
            if category == '1' or category == '2':
                break
            print('Wrong choice')
        ...
        if search_results:
            ...
            return "No notes or self.Tags were found"
