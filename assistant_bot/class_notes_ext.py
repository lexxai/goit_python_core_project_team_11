...
class Notes(UserDict):
    ...
    def sort(self, category):
        if category == "1":  # Datetime
            results = sorted(
                self.data.items(), key=lambda item: str(item[1].creation_date)
            )
        elif category == "2":  # Index
            results = sorted(self.data.items(), key=lambda item: item[1].index)
        elif category == "3":  # self.Tags
            results = sorted(
                self.data.items(), key=lambda item: [tag.value for tag in item[1].tags]
            )
        return results
    ...
    def show_notes(self, *args):
        if not self.notes:
            ...
            for record in iterator:
                ...
                if i % 4 == 0:
                    try:
                        line = input("Press 'Enter' to continue or 'Q' for end\n")
    ...
    def clear_notes(self, *args):
        if self.notes:
            if not args:
                while True:
                    choice = input(
                        'Press "y" to clear all records or "n" to discard: '
                    ).lower()
                    if choice in ["y", "n"]:
                        break
                    print("Wrong choice")
